...
hardwares.servos.servo1.setPosition(position);
...
| 改舵机开合位置、等待时间 | 修改 Auto 顶部 CLOSED_POSITION、OPEN_POSITION、SERVO_WAIT_SECONDS |
...
在 Hardwares 填舵机/Pinpoint 名称、Pinpoint 型号、方向与偏置；电机名称和方向目前沿用现有 MecanumDrive。Constants 填当前车的调参数据。Auto 填舵机两个 0~1 位置、等待秒数、功率和三个 Pose（x/y 为 inch，heading 为 rad）。
